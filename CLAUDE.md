...
- Phase 9 (Polish) — live. SheetJS dynamic-imported alongside react-pdf and pptxgenjs (entry chunk 1006 → 720 KB, 318 → 222 KB gz, ~30 % cold-start cut). Per-tab `ErrorBoundary` (class component with router-driven resetKey) wraps every route in TabShell so a worker / hydration crash stays contained. Global keyboard shortcuts via `useKeyboardShortcuts` (1–7 tab nav, R run engine, T run tornado, Esc clear layout selection) — input/textarea/contentEditable focus + modifier keys are honoured so shortcuts never fight text entry. Edge-case tests cover 1-SKU engagements, polygon envelopes that exclude every dock wall, automation systems that miss peak throughput, and zero-demand SKUs. SPEC §14 perf re-validated: 5 000-SKU pipeline runs well under the 50 ms budget; layout solver under 200 ms even at 500 SKU densities. Lint exits with 0 errors and 0 warnings (silenced the TanStack Table compiler warning at the call site with a justified disable).
- Build complete — production deployable.
...
## Phase 9 gate — verified

- [x] SheetJS code-split: OutputsTab dynamic-imports `src/exports/schedule-of-areas` on click. Entry chunk drops from 1006 KB / 318 KB gz to 720 KB / 222 KB gz (~30 % cold-start reduction). schedule-of-areas now a 289 KB / 97 KB gz lazy chunk alongside pdf-renderer (1.4 MB) and tornado-ppt (372 KB).
- [x] Error boundaries: `src/ui/components/ErrorBoundary.tsx` (class) wraps every route in TabShell with a tab-scoped fallback panel + router-driven `resetKey` so navigation clears errors automatically. Reset button on the panel restores the boundary in place.
- [x] Lint clean: pre-existing react-hooks/incompatible-library warning at LibraryTable.tsx:130 silenced with a justified eslint-disable-next-line. `npm run lint` now exits with 0 errors and 0 warnings for the first time since project start.
- [x] Keyboard shortcuts: `useKeyboardShortcuts` hook mounted at TabShell. Digits 1-7 navigate tabs; R / T dispatch CustomEvents that the Scenarios tab consumes for engine / tornado runs; Esc dispatches a clear-selection event the Layout tab uses to deselect zones. Shortcuts are no-ops while the user is in an input / textarea / contentEditable, and modifier keys (Ctrl/Meta/Alt) are honoured.
- [x] Edge cases verified: 1-SKU pipeline, polygon envelope excluding every dock wall, automation system missing peak throughput, zero-demand SKU suppression — all surface readable feasibility flags / suppressed counts instead of crashing.
- [x] SPEC §14 perf re-measured: 5 000-SKU pipeline well under the 50 ms budget (jsdom guard at 200 ms with headroom); layout solver under 200 ms at 500-SKU densities.
- [x] 16 new tests (ErrorBoundary × 4, keyboard shortcuts × 6, edge cases × 4, perf × 2); 187/187 total passing.
- [x] `npm run build` green; entry 720 KB / 222 KB gz. `npm run lint` green (0 errors, 0 warnings).
